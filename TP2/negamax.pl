...
negamax(J, Etat, P, Pmax, [Coup, Val]) :-
    successeurs(J,Etat,ListeS),
    loop_negamax(J,P,Pmax,ListeS,ListeC),
    meilleur(ListeC, [Coup, Val1]),
    Val is - Val1.
...
P -> profondeur
Pmax -> profondeur max
[[Coup,Suiv]|Succ] -> couples [Coup, Situation_Suivante]
[[Coup,Vsuiv]|Reste_Couples] -> couples [Coup, Valeur_Situation_Suivante]
*/
	/*
...
meilleur([[_C1, V1]|Succ], Best) :-
    meilleur(Succ, [C2, V2]), 
    V2 < V1, 
    Best = [C2, V2].

meilleur([[C1, V1]|Succ], Best) :- 
    meilleur(Succ, [_C2, V2]), 
    V2 >= V1, 
    Best = [C1, V1].
